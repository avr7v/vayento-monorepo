...
export default function BookingSuccessClient() {
  ...
  const confirmed =
    liveBookingStatus === 'CONFIRMED' || livePaymentStatus === 'SUCCEEDED';
    ...
    <main className="min-h-screen bg-[#f7f4ee] px-4 py-12">
      <div className="mx-auto max-w-4xl overflow-hidden rounded-[38px] border border-[#D7EBDD] bg-white shadow-[0_24px_70px_rgba(31,35,40,0.08)]">
        <div className="bg-[linear-gradient(135deg,#eefaf1_0%,#ffffff_55%,#e1f4e7_100%)] px-8 py-12 text-center sm:px-12">
          ...
          <h1 className="mt-4 text-5xl font-semibold text-[#1F2328]">
            Your booking is confirmed.
          </h1>

          <p className="mx-auto mt-5 max-w-2xl text-sm leading-8 text-[#5F5A53]">
            Thank you for choosing Vayento. Your payment was received and the
            reservation details have been sent to your email.
          </p>
        </div>

        <div className="p-8 sm:p-10">
          {isLoading ? (
            <p className="text-[#5F5A53]">Loading booking details...</p>
          ) : (
            <div className="rounded-[28px] border border-[#E8DED0] bg-white p-6">
              <div className="grid gap-5 sm:grid-cols-2">
                <Info
                  label="Booking status"
                  value={liveBookingStatus ?? 'Processing'}
                  highlight={confirmed}
                />

                <Info
                  label="Payment status"
                  value={livePaymentStatus ?? 'Processing'}
                  highlight={confirmed}
                />

                <Info
                  label="Property"
                  value={booking?.property?.title ?? 'Selected stay'}
                />
              ...
              {liveBookingStatus === 'AWAITING_PAYMENT' ? (
                <div className="mt-6 rounded-[20px] bg-[#FFF8E8] px-4 py-3 text-sm leading-7 text-[#8A6300]">
                  Stripe has accepted the payment request and the final webhook
                  confirmation may still be processing. Refresh the page in a
                  few seconds if needed.
                </div>
              ) : null}

              {!bookingId ? (
                <div className="mt-6 rounded-[20px] bg-red-50 px-4 py-3 text-sm leading-7 text-red-700">
                  No booking ID was provided in the URL.
                </div>
              ) : null}
            </div>
          )}
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Link
              href="/dashboard"
              className="rounded-full bg-[#1F2328] px-6 py-4 text-sm font-medium text-white"
            >
              View my bookings
            </Link>
            ...
            <Link
              href="/properties"
              className="rounded-full border border-[#1F2328] px-6 py-4 text-sm font-medium text-[#1F2328]"
            >
              Explore more stays
            </Link>
          </div>
...
function Info({
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 text-xl font-semibold text-[#1F2328]">
        {value}
      </div>
    </div>
  );
